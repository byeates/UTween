...
namespace Code.Common.Tween
{
	...
	/// </summary>
	public class UTweenTransformTransformMoveZ : UTweenTransform
	{
		//====================
		// PROTECTED
		//====================
		...
		public UTweenTransformTransformMoveZ
		(
			Transform target,
			float endValue,
			float time,
		...
		) : base(target, time, delay)
		{
			target = target;
			local = isLocal;
			current = local ? target.localPosition.z : target.position.z;
		...
		protected override void Init()
		{
			...
			if (local)
			{
				...
				result.z = target.localPosition.y;
			}
			else
			{
				result.x = target.position.x;
				result.z = target.position.y;
			}
		}
