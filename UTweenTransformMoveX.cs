...
namespace Code.Common.Tween
{
	...
	/// </summary>
	public class UTweenTransformTransformMoveX : UTweenTransform
	{
		//====================
		// PROTECTED
		//====================
		...
		public UTweenTransformTransformMoveX
		(
			Transform target,
			float endValue,
			float time,
		...
		) : base(target, time, delay)
		{
			target = target;
			local = isLocal;
			current = local ? target.localPosition.x : target.position.x;
		...
		protected override void Init()
		{
			...
			if (local)
			{
				result.y = target.localPosition.y;
		...
		protected override void ApplyEasing()
		{
			...
			result.x = easeMethod.Invoke(time, current, end, duration);
